...
class ListerProConfig extends Wire {
	...
	public function __get($name) {
		$value = $this->lister->$name; 
		if(is_null($value)) $value = parent::__get($name); 
		return $value; 
	}
	...
	public function ___buildForm() { 
		...
		$addNewListerURL = $config->urls->admin . "module/edit?name=ProcessPageListerPro#wrap__new_lister_title";
		...
		*/
		...
		// @todo make this an asmSelect multi-value selection 
		...
		if($page->name != 'lister') { 
			...
			if($input->post('createPermission')) {
		...
		/** @var InputfieldPageName $f */
		$f = $modules->get('InputfieldText');
		$f->attr('name', '_lister_name');
		$f->label = $this->_('Rename this lister');
		$f->description = $this->_('Use only characters `-_a-z0-9`'); 
		$f->pattern = '^[_a-zA-Z][-_a-z0-9]*$';
		$f->attr('placeholder', $page->name);
		$f->collapsed = Inputfield::collapsedYes;
		$f->icon = 'exchange';
		$form->add($f);
		...
		if($input->post('submit_save_config') || $input->post('submit_view')) {
	...
	protected function ___processInput(InputfieldForm $form) {
		...
		$configData = $modules->getModuleConfigData($this->lister);
		if(!isset($configData['settings'])) $configData['settings'] = array();
		
		$form->processInput($input->post); 
		$settings = array();
	
		$newName = $sanitizer->pageName($form->getChildByName('_lister_name')->val());
		$oldName = $page->name;
		$pageName = $oldName;
		
		if($newName && $newName != $oldName) {
			// rename lister
			$p = $page->parent()->child("name=$newName, include=all"); 
			if($p->id && $p->id != $page->id) {
				$this->error("Page $p->path already exists");
			} else if(isset($configData['settings'][$newName]) && $pages->get("template=admin, name=$newName")->id) {
				$this->error("Lister settings '$newName' already exists");
			} else {
				$page->of(false);
				$page->name = $newName;
				$page->save();
				$page->of(true);
				$pageName = $page->name;
				if(!isset($configData['settings'][$pageName]) && isset($configData['settings'][$oldName])) {
					$configData['settings'][$pageName] = $configData['settings'][$oldName];
					unset($configData['settings'][$oldName]);
				}
				$this->message("Renamed lister '$oldName' to '$pageName'"); 
			}
		}
		...
		$configData['settings'][$pageName] = $settings; 
		$this->lister->sessionClear();
		$modules->saveModuleConfigData($this->lister, $configData); 
		$this->message($this->_('Saved lister configuration ') . ' - ' . $pageName); 

		$newListerTitle = $input->post('_new_lister_title'); 
		if($newListerTitle) {
			$page = $this->lister->addNewLister($newListerTitle); 
			if($page->id) $session->redirect($page->url . 'config/');
		}

		if($input->post('submit_view')) {
			$session->redirect($page->url);
		} else {
			$session->redirect($page->url . '?s=1');
		}
	}
}
