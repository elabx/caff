...
var ProcessListerPro = {
	...
	init: function() {
		...
		$('#wrap_Inputfield_actions input').change(function() {
			var $form = $(this).closest('form');
			setTimeout(function() {
				// update html5 required property according to visibility
				$form.find('.InputfieldStateRequired :input').each(function() {
					if($(this).is(':visible')) {
	...
	pageClick: function() {
		...
		if(cnt > 0) {
			...
			$("#actions_items_all").prop('checked', false); 
			$("#actions_items_open")
				.removeAttr('disabled')
			...
			$("#actions_items_all").prop('checked', true); 
			if(!$wrap_actions_items.hasClass('InputfieldStateCollapsed')) {
